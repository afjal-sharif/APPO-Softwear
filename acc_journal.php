  ...
  if($_POST[pay_from]=='C-:CASH')
   {
     ...
     $sql="insert into tbl_account_coa(date,ref_id,remarks,deposite,withdraw,user,type) 
           value('$_POST[demo12]','$_POST[pay_head]','$remarks',0,$amount,'$_SESSION[userName]',2)"; 
     db_query($sql) or die(mysql_error());
   
     $remarks="Payabale Clear: Ref ID:". $_POST[pay_head]." ".$msgremarks;
     ...
     db_query($sql) or die(mysql_error());
     echo "<b><img src='images/active.png' height='15px' width='15px'> Success !! Payable Clear From Cash.</b>";
   }
   else
   {
    ...
    if($strtype=='B-:')
    {
     ...
     db_query($sql) or die(mysql_error());
     echo "<b><img src='images/active.png' height='15px' width='15px'> Success !! Payable Clear From Bank $finby</b>";
    }
   }
   } // Error chech If
 }// Submit If
...
<table width="960px" align="center" bordercolor="#AABBCC" bgcolor="#FFFFFF"  border=2 celspacing=1 cellpadding=5 style="border-collapse:collapse;">
 ...
 <tr id="trhead"><td colspan="5">Payable Journal Entry Form</td></tr>  
 
 <tr id="trsubhead"><td> Date</td><td>Payable Head</td><td> Pay From</td> <td>Amount</td></tr>
    <tr bgcolor="#CCAABB">  
       ...
       <td align="center">
         <?   
           $query_sql = "SELECT head_name,ref_id,sum(deposite-withdraw) as balance FROM tbl_account_coa 
                               join tbl_coa on ref_id=tbl_coa.id
                               where tbl_account_coa.type=2 and tbl_account_coa.exp_pro=0 group by ref_id";
           $sql = mysql_query($query_sql) or die(mysql_error());
           $row_sql = mysql_fetch_assoc($sql);
           $totalRows_sql = mysql_num_rows($sql);
         ?>
           <select  id="pay_head" name="pay_head"  style="width: 280px;">
           <option value=""></option> 
         <?
             do {  
         ?> 
            <option value="<?php echo $row_sql['ref_id'];?>"><?php echo $row_sql['head_name']?> :: Bal Amount :<?php echo number_format($row_sql['balance'],2)?>  </option>
