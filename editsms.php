...
<body bgcolor="#FFFFFF" width="1000px" text="#000000" link="#0000FF" vlink="#800080" alink="#FF0000">
 ...
 $flag=false;
  if(isset($_POST["submit"]))
   {
    $id=$_POST[id];
    $olddonumber=$_POST[olddonumber];
    
    
    if(empty($rec))
     {
      $sql="update tbl_order  set donumber='$_POST[donumber]',product=$_POST[product],qty=$_POST[qty],rate=$_POST[rate],comission=$_POST[comission],sp='$_POST[sp]' where id=$id";
     ...
     else
     {
       if($_POST[qty]>=$rec)
        ...
        $sql="update tbl_receive  set donumber='$_POST[donumber]' where donumber='$olddonumber'";
        db_query($sql) or die (mysql_error());
        $flag=true;
        $msg=$msg." Receive. ";

        $sql="update tbl_sales  set donumber='$_POST[donumber]' where donumber='$olddonumber'";
        db_query($sql) or die (mysql_error());
        $flag=true;
        $msg=$msg." Sales. ";
        
        $sql="update tbl_com_payment  set donumber='$_POST[donumber]' where donumber='$olddonumber'";
        db_query($sql) or die (mysql_error());
        $flag=true;   
        $msg=$msg." Payment. ";    
        }
 ...
 <? 
  ...
  <form name="vendor" id="vendor" action="" method="post">
    <table bgcolor="#FFEEFF" width="80%"  border="2" cellspacing="1" cellpadding="5" align="center" style="border-collapse:collapse;">
      ...
      <input type="hidden" name="olddonumber" value=<?=$value[donumber];?>>
      
      <tr  align="center"> 
       <td>Company</td>
       <td><input type="text"  name="company" size="50" value="<?=$value['name'];?>" readonly></td>
      </tr>
      <tr  align="center"> 
        <td>Product</td>
      ...
      <tr  align="center"> 
        ...
        <td>     
         <?
          $query_sql = "SELECT shortname  FROM tbl_sp order by id";
          $sql = mysql_query($query_sql) or die(mysql_error());
          $row_sql = mysql_fetch_assoc($sql);
          
        ?>
          <select name="sp" style="width: 130px;">
   ...
   <?}
   else
    {
      if($flag==true)
       {
        echo " <b>Update Successfully. || Please Click Referesh Button on Your Browser.</b>". $msg."<br>";
       ?>
